...
if(isset($_POST['trash']))
{
	...
	{
		$candel = "DELETE FROM `srthi_nhtcan` WHERE sr_empid='$trash' and sr_status IN('0','6','9')";
				...
				<div class="clearfix">
					<form class="form-inline" method="GET" action="">
						<select name="tainer" class="form-control">
							...
							<?php
								$bsql = "SELECT DISTINCT sr_heademail, sr_headname FROM srthi_nhtcan WHERE sr_status='1'";
						...
						<div class="table-wrapper-scroll-y my-custom-scrollbar">
							<table class="table table-bordered table-striped table-hover">
								...
								if(isset($_GET['tainer']))
								{
									...
									$num = 1;
									$sql = "SELECT * FROM `srthi_nhtcan` WHERE sr_heademail='$tainer' AND sr_status IN ('0','6','9')";
								...
								else
								{
									...
									$num = 1;
									$sql = "SELECT * FROM `srthi_nhtcan`WHERE sr_status IN ('0','6','9')";
									$res = mysqli_query($conn,$sql);
									while($row = mysqli_fetch_array($res))
									{
										?>

...
if(isset($_POST['assignsub']))
{
	...
	$srtestyp = $tnrow['sr_test_type'];
		...
		{
			$rsql = "SELECT * FROM srthi_nhtattend WHERE sr_empid='$trash'";
			...
			while($rrow = mysqli_fetch_array($rres))
			{
				...
				$empsubpro = $rrow['sr_suprocess'];

				$ass = "INSERT INTO `srthi_assign_test`(`sr_test_id`, `sr_testname`, `sr_empid`, `sr_empname`, `sr_emprocess`, `sr_empsubpro`, `sr_batch`, `sr_assignby`, `sr_heademail`, `sr_test_status`, `sr_testype`, `sr_result`) VALUES ('$testid','$srtname','$trash','$emname','$emproce','$empsubpro','$embtach','$tname','$temail','1','$srtestyp','Not Attempt')";
				
				$ares = mysqli_query($conn,$ass);
				
				if($ares == true)
				{
					$msg = "<div class='alert-success'>Test Assign Successfully!</div>";
				}
				else
				{
					$msg = "<div class='alert-warning'>Something Went Wrong!</div>";
				}
			}
...
<section class="main-branch">
	...
	<div class="content-bar">
		<div class="container">
			<div class="row py-4">
				<div class="col-lg-4">
					<h5 class="content-head-top"><span><a href="javascript:history.go(-1)"><img src="../assets/img/back.png"></a></span> Exist Assign Test</h5>
				</div>
				<div class="col-lg-8">
					<form class="form-inline" method="GET" action="">
						<select name="batch" class="form-control mr-3">
							...
							<?php
								$sql = "SELECT DISTINCT sr_batch FROM srthi_nhtattend WHERE sr_heademail='$temail' AND sr_attendstatus IN ('0','1') AND sr_batchtype='Exist'";
						...
						<select name="pro" class="form-control mr-3">
							...
							<?php
								$sql = "SELECT DISTINCT sr_process FROM srthi_nhtattend WHERE sr_heademail='$temail' AND sr_attendstatus IN ('0','1') AND sr_batchtype='Exist'";
								$res = mysqli_query($conn,$sql);
								while($row = mysqli_fetch_array($res))
								{
									?>
						...
						<select name="subpro" class="form-control">
							...
							<?php
								$sql = "SELECT DISTINCT sr_suprocess FROM srthi_nhtattend WHERE sr_heademail='$temail' AND sr_attendstatus IN ('0','1') AND sr_batchtype='Exist'";
								$res = mysqli_query($conn,$sql);
								while($row = mysqli_fetch_array($res))
								{
									?>
									<option value="<?php echo $row['sr_suprocess'];?>"><?php echo $row['sr_suprocess'];?></option>
									<?php
								}
							?>
						</select>
				...
				<table class="table table-bordered table-striped table-hover">
					...
					<tbody>
						...
						if(isset($_GET['submit']))
						{
							...
							$subpro = $_GET['subpro'];
							$num = 1;
							$fsql = "SELECT DISTINCT sr_empid,sr_batch,sr_name,sr_process,sr_suprocess FROM srthi_nhtattend WHERE sr_batch='$batch' AND sr_process='$pro' AND sr_suprocess='$subpro' AND sr_heademail='$temail' AND sr_attendstatus IN ('0','1') AND sr_batchtype='Exist'";
						...
						{
							$num = 1;
							$fsql = "SELECT DISTINCT sr_empid,sr_batch,sr_name,sr_process,sr_suprocess FROM srthi_nhtattend WHERE sr_heademail='$temail' AND sr_attendstatus IN ('0','1') AND sr_batchtype='Exist'";
							$fres = mysqli_query($conn,$fsql);
							while($frow = mysqli_fetch_array($fres))
							{
								?>
